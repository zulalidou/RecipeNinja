...
const getSearchedRecipes = async (foodSearched) => {
    try {
      const response = await fetch(`/api/get-searched-recipes?foodSearched=${foodSearched}`, {
        method: "GET",
        headers: {
    ...
    useEffect(() => {
        const fetchRecipes = async () => {
            ...
            if (searchedRecipes === undefined) {
                ...
                db.recipes.add({searchTerm: location.state.foodSearched, recipes: searchedRecipes})
            }
            else {
    ...
    return (
        <div className='body'>
            {(searchResults === null) ?
                ...
                <div>
                    <h1 className='search-results-header'>{searchResults.length} results found for <em>{location.state.foodSearched}</em></h1>
                    {searchResults.length === 0 &&
                        <div>
                            <p className='no-search-results'>Please check your spelling and try again</p>
                            <p className='no-search-results'>Try more general words</p>
                        </div>
